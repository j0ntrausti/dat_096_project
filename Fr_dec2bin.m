...
precision = 24 ;            % See the note above.
...
if length(Fr_bin) == 26  &  all ( Fr_bin == '.0000000000000000000000000' )
...
% the 2nd expr is not computed if the 1st expr is false.
    ...
    if Fr_dec_Current >= 2^-(26)
        % Fr_bin = strcat ( Fr_bin,  repmat ( ['0'], 1, 16 ),  '1'  ) ; % Old
        Fr_bin = strcat ( Fr_bin,  '1'  ) ;
        
        Fr_dec_Current = Fr_dec_Current - 2^-(26) ;
        if Fr_dec_Current  >=  2^(-27)
            Fr_bin = strcat (  Fr_bin, '1' ) ;
        end
        ...
        Fr_dec_Current = Fr_dec_Current - 2^-(27) ;
        if Fr_dec_Current  >=  2^(-28)
            Fr_bin = strcat (  Fr_bin, '1' ) ;
        end
    end
